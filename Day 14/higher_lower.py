...
while score != 0:
    ...
    print(
        f"Comapre A: {initial['name']}, a {initial['description']}, from {initial['country']}")
    print(art.vs)
    # Genrating and printing second entry
    second_rn = randint(0, len(data)-1)
    second = data[second_rn]
    print(
        f"Comapre B: {data[second_rn]['name']}, a {data[second_rn]['description']}, from {data[second_rn]['country']}")
